...
Lastly, the code functions (sourced .Rmd files) also contain details in the form of comments to explain the relevant code chunks and their purpose.
...
## 1. Log Returns
Log return for Analytics Ci Moderate Fund of Funds are first calculated from prices.
```{r Dlog returns}
...
Fitting the GARCH model requires cumulative returns, hence cumulative returns for Analytics Ci Moderate Fund of Funds are calculated from the log returns.
```{r}
...
The residuals are filtered to produce close to i.i.d Stationary standardized residuals.
```{r}
...
100 simulation pathways of one week are simulated from 14 September 2018 to 20 September 2018.
```{r}
...
100 simulation paths of one month are simulated from 14 September 2018 to 15 October 2018.
```{r}
...
100 simulation paths of two months are simulated from 14 September 2018 to 15 November 2018.
```{r}
...
The VaR is calculated for the simulated pathways of the three time periods.
```{r}
...
Using the historical values for the one-week, one-month and two-month time periods (14 September 2018 to 20 September 2018, 14 September 2018 to 15 October 2018 and 14 September 2018 to 15 November 2018.), the VaR is calculated to compare with the VaR of the simulated pathways. Thus, the predictive strength of the FHS in determining VaR of can be compared with actual data.
```{r}
...
A table is created to directly compare the VaR's of the simulated periods (first row) and the VaR of the historical data (second row) for those periods. Furthermore, the third row shows the difference in absolute values between the VaR calculated from simmulations and the VaR calculated from the historical data.

NB! this table is produced in LateX for the PDR write-up and will not be shown nicely in the README.
