...
# Calculating dlog returns from prices: ## simple returns or Dlog returns
DLog_func <- function(x) {x %>% 
    group_by(FundName) %>% 
    arrange(Date) %>% 
    mutate(Return = (log(Price)-log(lag(Price) - 1))*100) %>%
...
# STANLIB Global Balanced Feeder Fund B:
...
H1_GARCH_func <- function(x){
  ...
  H1_fit = ugarchfit(spec = garch11, data = as.numeric(x), solver = "solnp")
...
# Simulation
mysim_H1 <- ugarchsim(H1_fit, n.sim = 60, m.sim =1000, startMethod = "sample")

# obtain fitted log returns
H1_sim <- fitted(mysim_H1)
# turn into data frame
H1_sim <- as.data.frame(H1_sim) 
